...
class CollisionLine:
    ...
    def draw(self, game) -> None:
        """绘制线段到游戏窗口"""
        if self.display:
            pygame.draw.line(game.screen, "black", (game.realToScreen(self.start.x, game.x), game.realToScreen(
                self.start.y, game.y)), (game.realToScreen(self.end.x, game.x), game.realToScreen(self.end.y, game.y)))
...
class Element:
    ...
    def isMouseOn(self, game) -> bool:
        """检测鼠标是否在元素上"""
        pos = Vector2(game.screenToReal(pygame.mouse.get_pos()[
...
class Coordinator():
    ...
    def draw(self, game, option, text: str = "") -> None:
        ...
        for direction in self.direction:
            pygame.draw.line(game.screen, "black", (game.realToScreen(self.position.x, game.x), game.realToScreen(self.position.y, game.y)), (
                game.realToScreen(self.position.x + direction.x, game.x), game.realToScreen(self.position.y + direction.y, game.y)))

        self.showDegree(game, Vector2(game.screenToReal(pygame.mouse.get_pos()[
                        0], game.x), game.screenToReal(pygame.mouse.get_pos()[1], game.y)), option, text)
...
class Ball(Element):
    ...
    def copy(self, game) -> None:
        ...
        while isMoving:
            newBall.position = Vector2(game.screenToReal(pygame.mouse.get_pos(
            )[0], game.x), game.screenToReal(pygame.mouse.get_pos()[1], game.y))
    ...
    def reboundByLine(self, line: CollisionLine, timeIsReversed: bool = False) -> Vector2:
        ...
        self.displayedVelocity = self.velocity + \
            (self.displayedVelocity - self.velocity) * \
            self.displayedVelocityFactor
        ...
        if line.isLine:  # 直线的情况
            ...
            # 根据球的位置确定法线方向
            normal = edgeNormal if (
                self.position - closest).dot(edgeNormal) > 0 else -edgeNormal
        ...
        if overlap > 0:
            ...
            self.velocity = self.velocity.copy().normalize() * abs(abs(self.velocity) **
                                                                   2 - 2 * 98.1 * (1 - cosine ** 2) ** 0.5 * overlap) ** 0.5
                ...
                if abs(gravityComponent) > 0.01:
                    ...
                    if gravityComponent > 0:  # 下方球体需要更强的向上分离力
                        self.velocity += normal * \
                            (min_sep_speed * sepFactor + gravityBoost) * \
                            (ball.mass / totalMass)
                        ball.velocity -= normal * \
                            (min_sep_speed * sepFactor) * \
                            (self.mass / totalMass)
                    else:  # 上方球体需要更强的向下分离力
                        self.velocity += normal * \
                            (min_sep_speed * sepFactor) * \
                            (ball.mass / totalMass)
                        ball.velocity -= normal * \
                            (min_sep_speed * sepFactor + abs(gravityBoost)) * \
                            (self.mass / totalMass)
                else:
                    # 水平方向的普通分离
                    self.velocity += normal * min_sep_speed * \
                        sepFactor * (ball.mass / totalMass)
                    ball.velocity -= normal * min_sep_speed * \
                        sepFactor * (self.mass / totalMass)
    ...
    def merge(self, other: Self, game) -> Self:
        ...
        totalPosition = (self.position * self.mass +
                         other.position * other.mass) / (self.mass + other.mass)
        totalVelocity = (self.velocity * self.mass +
                         other.velocity * other.mass) / (self.mass + other.mass)
        ...
        totalColor = colorTupleToString(colorMiddle(
            self.color, other.color, self.radius / totalRadius))

        newBall = Ball(totalPosition, totalRadius, totalColor, totalMass,
                       totalVelocity, totalForce, gravitation=game.isCelestialBodyMode)
        ...
        newBall.displayedAcceleration = self.displayedAcceleration + \
            other.displayedAcceleration
...
class Wall(Element):
    ...
    def __init__(self, vertexes: list[Vector2], color: pygame.Color, isLine: bool = False) -> None:
        ...
        self.lines: list[CollisionLine] = [
            CollisionLine(vertexes[0], vertexes[1], isLine), CollisionLine(
                vertexes[1], vertexes[2], isLine),
            CollisionLine(vertexes[2], vertexes[3], isLine), CollisionLine(
                vertexes[3], vertexes[0], isLine)
        ]
    ...
    def copy(self, game) -> None:
        ...
        while isMoving:
            newWall.position = Vector2(game.screenToReal(pygame.mouse.get_pos(
            )[0], game.x), game.screenToReal(pygame.mouse.get_pos()[1], game.y))
            newWall.draw(game)
            newWall.update(0.5)
            game.update()
    ...
    def update(self, deltaTime: float) -> Self:
        ...
        self.lines = [
            CollisionLine(self.vertexes[0], self.vertexes[1], self.isLine), CollisionLine(
                self.vertexes[1], self.vertexes[2]),
            CollisionLine(self.vertexes[2], self.vertexes[3]), CollisionLine(
    ...
    def draw(self, game) -> None:
        ...
        if self.highLighted:
            ...
            pygame.draw.polygon(game.screen, (255, 255, 0), [(game.realToScreen(
                hightLight.x, game.x), game.realToScreen(hightLight.y, game.y)) for hightLight in highLightList], 0)

        pygame.draw.polygon(game.screen, self.color, [(game.realToScreen(
            vertex.x, game.x), game.realToScreen(vertex.y, game.y)) for vertex in self.vertexes], 0)
        self.highLighted = False
...
class Rope(Element):
            ...
            if isinstance(self.start, Ball) and isinstance(self.end, Ball):
                ...
                if relVel > -minSepSpeed:
                    ...
                    if abs(gravityComponent) > 0.01:
                        ...
                        # 水平方向的普通分离
                        self.start.velocity += normal * minSepSpeed * \
                            sepFactor * (self.end.mass / totalMass)
                        self.end.velocity -= normal * minSepSpeed * \
                            sepFactor * (self.start.mass / totalMass)
    ...
    def draw(self, game) -> None:
        """绘制绳索"""
        pygame.draw.line(game.screen, self.color, (game.realToScreen(self.start.getPosition().x, game.x), game.realToScreen(self.start.getPosition(
        ).y, game.y)), (game.realToScreen(self.end.getPosition().x, game.x), game.realToScreen(self.end.getPosition().y, game.y)), self.width)
