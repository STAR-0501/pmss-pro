...
- 重力设为0模拟太空失重
- 调整碰撞系数实现不同弹性碰撞效果
- 环境设置面板数值与当前环境参数同步显示（加载预设后也同步）
- 关闭面板（M/Enter/Esc）时自动提交本次输入
...
- 环境设置面板打开时与当前环境参数实时同步，解决加载预设后数字不更新的问题。
- 关闭环境面板时自动提交本次输入（支持 M / Enter / Esc）。
- 数值更改面板中的电荷量显示保留一位小数。

## ⚡ 电荷与库仑力（详解）

- 属性定义：每个球体包含电荷量 `electricCharge`，用于模拟库仑力作用；创建时可在右侧菜单设置，运行中可通过属性编辑面板调整。
- 显示与输入：
  - 跟随提示与信息面板的电荷数值均以一位小数显示，便于观测。
  - 数值更改面板中电荷的输入框初始文本同样以一位小数显示，仅影响展示，不改变内部计算精度。
- 物理模型：
  - 力大小按近似库仑定律计算：`F = k * q1 * q2 / (r^2 + ε)`，其中 `k` 为工程化缩放常数，`ε` 为防止除零的极小量；同号电荷相互排斥，异号电荷相互吸引。
  - 数值稳定性：设定了最小距离钳制（避免过近导致力过大），并将电力作为“自然外力”参与加速度计算，与引力等共同作用。
  - 与引力兼容：电荷力与引力可同时启用，二者的合力决定运动轨迹。
- 融合行为：
  - 在天体融合发生时，新球的电荷量等于两者电荷之和；可用于模拟带电天体/粒子合并后的电荷守恒。
- 预设复现：
  - 目前预设保存未包含电荷值（实验复载后需手动设置电荷）；如需将电荷一并保存/恢复，可在后续版本开启该项。
- 使用建议：
  - 为突出电荷效应，可增大电荷量、减小质量或降低空气阻力；观察排斥/吸引导致的轨迹变化。
  - 与天体模式结合：在启用引力的同时设置电荷，探索二力竞争下的稳定/不稳定轨道。
